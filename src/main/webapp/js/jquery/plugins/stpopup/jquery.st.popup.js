//弹出层主函数
function popup(popParam){

//内建弹出层对象
var popUp=new PopupLayer(popParam);

//当焦点离开层的时候，进行处理弹出层效果
$(popParam.trigger).bind("blur",function(){
	//当点击事件发生时，查看点击层上的元素
	document.onclick = function(e){
		var el = e.srcElement || e.target;
		var divClass=popParam.trigger;
		
		//遍历层上所有带popup的class,
		$.each($(popParam.popupBlk+" .popup"),function(i,val){
			divClass=divClass+($(val.outerHTML).attr("id")+',');
		});
		
		//判断当前点击的对象是否在层上，如果不在层上且默认关闭打开，则关闭层
		if(divClass.indexOf(el.id)<=0&&popParam.isAutoClose){
			popUp.close();
		}
	}
  }.binding(this));
