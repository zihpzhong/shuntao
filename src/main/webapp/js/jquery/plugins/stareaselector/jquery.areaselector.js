...
counties["成都"] = new Array("成华", "崇州", "大邑", "都江堰", "锦江", "金牛", "龙泉驿", 
		"彭州" , "郫县" , "浦江" , "青白江" , "青羊" , "邛崃" , "双流" , "温江" , "武侯" , "新都" , "新津") ;
;
(function($){
    jQuery.fn.extend({
        "citypicker": function(){
    ...
    jQuery.CityPicker = function(obj){
        ...
        function setupContainer(){
            ...
            $prov.addClass("list_wrap");
            var $city = $(city).attr("id", "citypicker_city");
            $city.addClass("list_wrap");
            var $county = $(county).attr("id" , "citypicker_county") ;
            $county.addClass("list_wrap");
            $container.append($caption).append($prov).append($city).append($county);
            
            $("<strong>快速下单</strong>").appendTo($caption);
            $("<span href='javascript:void;' style='float:right;cursor:hand;'>关闭</span>").appendTo($caption).click(function(){
                $container.slideUp(100);
            });
            for (p in pro) {
                $("<p class='hot'>" + pro[p] + "</p>").appendTo($prov).click(function(){
                    var i = $(this).text();
                    //被点了的省份字体色改变
                    $(this).css({
                        'color': 'blue',
                    ...
                    $city.empty();
                    for (c in cities[i]) {
                        $("<p class='hot'>" + cities[i][c] + "</p>").appendTo($city).click(function(){
                        	 var j = $(this).text();
                             //被点了的省份字体色改变
                             $(this).css({
                                 'color': 'blue',
                             ...
                             //清空城市，让另外一个省的城市显示在此处
                             $county.empty();    
                        	 for(county_ in counties[j]){
                        		 $("<p class='hot'>" + i + j + counties[j][county_] + "</p>").appendTo($county).click(function(){
                        			//这里把相应的值设置到输入框当中去
                                     $(obj).val($(this).text());
                                     //被点了的城市字体色改变
                                     $(this).css({
