...
/*
 * jQuery文件上传插件,封装UI,上传处理操作采用Baidu WebUploader; @Author jh;
 */
(function( $ ) {
    ...
    $.fn.extend({
        ...
        diyUpload:function( opt, serverCallBack ) {
			...
			webUploader.on('uploadProgress',function( file, percentage){
				...
				showDiyProgress( percentage.toFixed(2), $diyBar,"",null,webUploader);
			});
			...
			webUploader.on('uploadError',function( file, reason ){
				var reUploadFileId='fileBox_'+file.id;
				var $fileBox = $('#'+reUploadFileId);
				var $diyBar = $fileBox.find('.diyBar');
				
				showDiyProgress(0, $diyBar, '上传失败！重新上传',file,webUploader);
	...
	// 操作进度条;
	function showDiyProgress( progress, $diyBar, text, reUploadFile,webUploader) {
		if ( progress >= 100 ) {
			progress = progress + '%';
			text = text || '上传完成';
		} else {
		...
		if(reUploadFile!=null){
			$diyProgressText.bind('dblclick',function(){
				webUploader.retry(reUploadFile);
			});
		}
	...
	function saveOrder() {
		var picOrder=$("#picList li").map(function() { return $(this).children().html(); }).get();
		$("input[name=picListOrder]").val(picOrder.join("|"));
	}
	...
	function createBox( $fileInput, file, webUploader,opt ) {
		...
		if ( $parentFileBox.length <= 0 ) {
			...
			var div = '<div class="parentFileBox"> \
						<ul id="picList" class="fileBoxUl" ></ul>\
					   </div>';
			$fileInput.after( div );
			$parentFileBox = $fileInput.next('.parentFileBox');

			$parentFileBox.find("#picList").dragsort({ dragSelector: "div", dragBetween: true, dragEnd: saveOrder, placeHolderTemplate: "<li class='placeHolder'><div></div></li>" });
		}
		...
		// 添加子容器;
		var customWidth=parseInt(opt.width)-10;
		var customHeight=parseInt(opt.height);
		var li = '<li id="fileBox_'+file_id+'" style="width:'+customWidth+'px;height:'+customHeight+'px" class="diyUploadHover"> \
		            <div class="diyFileName" style="width:'+(parseInt(opt.width)-10)+'px;">'+file.name+'</div>\
		...
		// 父容器宽度;
		var $width = $('.fileBoxUl>li').length * parseInt(opt.width);
