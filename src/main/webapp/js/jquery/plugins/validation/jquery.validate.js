...
/**
 * 文档页面：http://jqueryvalidation.org/jQuery.validator.format/
 * jQuery.validator.format()：
 * 		jQuery.validator.format( template, argument, argumentN… )  
 * 		Description: Replaces {n} placeholders with arguments. 
 * 		template			Type: String		The string to format.
 * 		argument			Type: Object		The first argument to insert, or an array of Strings to insert
 * 		argumentN…	Type: Object		The second etc. argument to insert
...
$.validator.format = function( source, params ) {
	...
	if ( arguments.length === 1 ) {
		return function() {
			 ...
			 * 		概述：将类数组对象转换为数组对象。类数组对象有 length 属性，其成员索引为 0 至 length - 1。实际中此函数在 jQuery 中将自动使用而无需特意转换。
			 * 		参数：obj，Object，类数组对象。
			 */
	...
	if ( arguments.length > 2 && params.constructor !== Array  ) {
		/**
		 * slice(start, [end])：
		 * 		概述：选取一个匹配的子集。与原来的slice方法类似。
		 * 		参数：start，Integer，开始选取子集的位置。第一个元素是0.如果是负数，则可以从集合的尾部开始选起。
...
$.extend( $.validator, {
	...
	/**
	 * 文档页面：http://jqueryvalidation.org/jQuery.validator.setDefaults/
	 * jQuery.validator.setDefaults()：
	 * 		jQuery.validator.setDefaults(options) 
	 * 		Description: Modify default settings for validation.
	 * 		options			Type: Object		Options to set as default.
	...
	prototype: {
		...
		/**
		 * 文档页面：http://jqueryvalidation.org/Validator.form/
		 * Validator.form()：
		 * 		Validator.form() 
		...
		/**
		 * 文档页面：http://jqueryvalidation.org/Validator.element/
		 * Validator.element()：
		 * 		Validator.element()
		 * 		Description: Validates a single element, returns true if it is valid, false otherwise.
		 * 		element		Type: Selector		An element to validate, must be inside the validated form.
		 * 		Example:
		 * 			var validator = $( "#myform" ).validate();
		...
		/**
		 * 文档页面：http://jqueryvalidation.org/Validator.showErrors/
		 * Validator.showErrors()：
		 * 		Validator.showErrors(errors)
		 * 		Description: Show the specified messages.
		 * 		errors		Type: Object		One or more key/value pairs of input names and messages.
		 * 		Example:
		...
		/**
		 * 文档页面：http://jqueryvalidation.org/Validator.resetForm/
		 * Validator.resetForm()：
		 * 		Validator.resetForm()
	...
	methods: {
		...
		/**
		 * 文档页面：http://jqueryvalidation.org/required-method/
		 * required method：
		 * 		required()			Returns: Boolean 
		 * 		Description: Makes the element required. 
		 * 		required( dependency-expression )
		 * 		dependency-expression			Type: String				An expression (String) that is evaluated in the context of the element's form, 
		 * 				making the field required only if the expression returns more than one element.
		 * 		required( dependency-callback )
		 * 		dependency-callback				Type: Function()		The function is executed with the element as it's only argument: 
		 * 				If it returns true, the element is required. 
